...
def PerformEMEngToGer(training_sentences, epochs):
	...
	for i in range(0, epochs):
		...
		for (ger_sentence, eng_sentence) in training_sentences:
			...
			for eng_word in eng_words:
				for ger_word in ger_words:
					val = (float)(trans_e_f[(eng_word, ger_word)])/(float)(s_total_e[eng_word])
					count_e_f[(eng_word, ger_word)] += val
					total_f[ger_word] += val
...
PerformEMEngToGer(training_sentences, 15)
