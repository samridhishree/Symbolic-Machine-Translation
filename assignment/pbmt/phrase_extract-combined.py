...
raw_alignment_file = sys.argv[3]
neural_alignment_file = sys.argv[4]
phrase_file = sys.argv[5]
max_phrase_len = int(sys.argv[6])
count_f_e = defaultdict(int)
count_e = defaultdict(int)
score_f_e = defaultdict(float)
raw_align_file = open(raw_alignment_file, 'r')
raw_alignments = raw_align_file.readlines()
neural_align_file = open(neural_alignment_file, 'r')
neural_alignments = neural_align_file.readlines()

#Function to create the alignment dictionary (eng->[german]) and (ger->[english]) for ith sentence
def CreateAlignments(alignments):
	sentence_alignments = alignments.strip().strip("\n")
	if sentence_alignments == "NO ALIGNMENTS FOUND" or sentence_alignments == "":
		sentence_alignments = []
	else:
		sentence_alignments = sentence_alignments.split(" ")
...
for i in range(len(training_sentences)):
	...
	#e_align_dict_raw, f_align_dict_raw = CreateAlignments(raw_alignments[i])
	if i%1000 == 0:
		print "Phrase for alignment ", i
	#Raw Alignments
	phrases = PhraseExtract(eng_sent, ger_sent, e_align_dict_raw, f_align_dict_raw)
	UpdatePhraseCounts(phrases)
	#Neural Alignments
	e_align_dict_neu, f_align_dict_neu = CreateAlignments(neural_alignments[i])
	phrases = PhraseExtract(eng_sent, ger_sent, e_align_dict_neu, f_align_dict_neu)
	UpdatePhraseCounts(phrases)
